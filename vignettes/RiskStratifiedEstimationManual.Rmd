...
```
We can produce the covariate balance plots within all risk strata and combine them in a single plot using the code below.
```{r eval = FALSE}


```
## Weighted Kaplan-Meier
Using the code presented below we can produce plots of the weighted Kaplan-Meier estimates within risk strata. The time interval of interest id one year (365 days). Again, we consider the case of the 4th risk stratum. 
...
```
One can produce plots for all risk strata and combine them in the same way as before.
```{r eval = FALSE}
